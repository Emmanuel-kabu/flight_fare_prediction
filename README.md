...
An end-to-end machine learning system that predicts domestic and international flight fares from Bangladesh using a **CatBoost** regressor, deployed with a full MLOps stack including experiment tracking, model registry, automated retraining, and performance monitoring.
...
- **Algorithm:** CatBoost Regressor with native categorical handling
...
- **Hyperparameters:** depth=8, learning_rate=0.02, 2000 iterations, early stopping (100 rounds)
...
The Airflow DAG (`flight_fare_ml_pipeline`) runs **weekly** and executes:
...
1. **Data Extraction** — loads raw CSV into Parquet
2. **Preprocessing** — temporal split, feature engineering, frequency encoding
3. **Training** — CatBoost with early stopping on validation set
4. **Evaluation** — computes R², MAE, RMSE, MAPE, MedAE
5. **MLflow Logging** — logs params, metrics, model, and registers in Model Registry
6. **Evidently Monitoring** — generates data drift & concept drift reports
7. **Notification** — prints summary with all metrics
